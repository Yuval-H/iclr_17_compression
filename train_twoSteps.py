...
val_every = 25000
save_every = 2000
hammingLossOnBinaryZ = True
start_from_pretrained = '/home/access/dev/iclr_17_compression/checkpoints_new/not-binary-try/rec/iter_0_111.pth.tar'
save_path = '/home/access/dev/iclr_17_compression/checkpoints_new/N=1024/two_steps(using pre-trained-rec)'
...
#tsfm = transforms.Compose([transforms.CenterCrop((368,1216)),transforms.ToTensor()])
...
# Load model - Compression baseline model:
...
# Load model - Compression latent model
model_comp = ImageCompressor_small()
start_from_pretrained2 = ''
if start_from_pretrained2 != '':
    global_step_ignore = load_model(model_comp, start_from_pretrained2)
model_comp = model_comp.to(device)
model_comp.train()

# todo: check with & without
clipping_value = 5.0
torch.nn.utils.clip_grad_norm_(model_comp.parameters(), clipping_value)
...
optimizer = torch.optim.Adam(model_comp.parameters(), lr=lr_start)
#optimizer = torch.optim.SGD(model_comp.parameters(), lr=lr_start, weight_decay=1e-8, momentum=0.9)
...
for epoch in range(1, n_epochs + 1):
    ...
    for batch, data in enumerate(train_dataloader):
        ...
        images_cam1 = images_cam1[:, :, :16 * (shape[2] // 16), :16 * (shape[3] // 16)]
        images_cam2 = images_cam2[:, :, :16 * (shape[2] // 16), :16 * (shape[3] // 16)]
        ...
        # Maybe use this if training the baseline auto-encoder
        ...
        outputs_cam1, z_cam1, _ = model(images_cam1)
        outputs_cam2, z_cam2, _ = model(images_cam2)
        recon_z1, z1_down, mse_loss = model_comp(z_cam1, z_cam2)


        #rec_im_loss = criterion(outputs_cam1, images_cam1) + criterion(outputs_cam2, images_cam2)
        #rec_z_loss = criterion(z_cam1, recon_z1)
        #loss = rec_im_loss + 0.1*rec_z_loss
        loss = mse_loss
    ...
    if train_loss < best_loss:
        ...
        save_model(model_comp, 1, save_path) #save_model(model, epoch, save_path)#torch.save(model.state_dict(), '../model_best_weights.pth')
    elif epoch % save_every == 0:
        save_model(model_comp, epoch, save_path)
