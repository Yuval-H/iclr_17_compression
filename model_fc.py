...
def load_model(model, f):
    ...
    f = str(f)
    if 0:#f.find('iter_') != -1 and f.find('.pth') != -1:
        st = f.find('iter_') + 5
        ed = f.find('.pth', st)
        return int(f[st:ed])
    else:
...
class ImageCompressor_fc(nn.Module):
    def __init__(self, out_channel_N=64):
        super(ImageCompressor_fc, self).__init__()
    ...
    def forward(self, input_image):
        ...
        ## using fc layer:
        temp_size = compressed_feature_renorm.size()
        final_encoder = self.fc(compressed_feature_renorm.view(batch_size,-1))


        recon_image = self.Decoder(final_encoder.view(temp_size))
        ...
        return clipped_recon_image, compressed_feature_renorm, bpp_feature
