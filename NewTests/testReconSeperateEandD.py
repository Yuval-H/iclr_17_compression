...
pretrained_model_path_encoder = '/home/access/dev/iclr_17_compression/checkpoints_new/not-binary-try/rec+dist (start from rec pretrained)/iter_637.pth.tar'
pretrained_model_path_decoder = '/home/access/dev/iclr_17_compression/checkpoints_new/not-binary-try/rec+dist (start from rec pretrained)/decoder-only/iter_1697.pth.tar'

device = 'cuda' if torch.cuda.is_available() else 'cpu'
# Load Encoder:
model = ImageCompressor()
global_step_ignore = load_model(model, pretrained_model_path_encoder)
model = model.to(device)
Encoder = model.Encoder
# Load Decoder
model2 = ImageCompressor()
Decoder = model2.Decoder.to(device)
global_step_ignore = load_model(Decoder, pretrained_model_path_decoder)
...
for i in range(len(stereo1_path_list)):
    ...
    encoded = torch.round(Encoder(input1))
    clipped_recon_image = Decoder(encoded)
